...
var DATA_STREAMS_ENDPOINT = "https://api.testnet-dataengine.chain.link";
var DEFAULT_FEED = "0x000359843a543ee2fe414dc14c7e7920ef10f4372990b79d6361cdc0dd1ba782";
var fetchPriceOnNode = (sendRequester, config, creds) => {
  ...
  if (!report2 || !report2.price) {
    throw new Error("No price in report body");
  }
  const rawPrice = BigInt(report2.price);
  return Number(rawPrice) / 1000000000000000000;
};
function fetchSusdPrice(runtime2) {
  const httpClient = new ClientCapability2;
  try {
    const clientId = runtime2.getSecret({ id: "DATA_STREAMS_CLIENT_ID" }).result().value;
    const clientSecret = runtime2.getSecret({ id: "DATA_STREAMS_CLIENT_SECRET" }).result().value;
    let feedId = DEFAULT_FEED;
    try {
      const secretFeed = runtime2.getSecret({ id: "DATA_STREAMS_FEED_ID" }).result().value;
      if (secretFeed)
        feedId = secretFeed;
    } catch (e) {}
    const creds = { clientId, clientSecret, feedId };
    const price = httpClient.sendRequest(runtime2, fetchPriceOnNode, consensusMedianAggregation())(runtime2.config, creds).result();
    return {
      price,
      source: "data-streams",
    ...
    runtime2.log(`[peg-monitor] Data Streams aggregation failed: ${err.message}`);
  }
  return {
    price: 1,
    source: "default",
...
init_toHex();
var DEFAULT_MAX_ACTION_AMOUNT = BigInt("10000000000000000000000");
function dispatchAction(runtime2, priceResult) {
  ...
  runtime2.log(`[action-dispatcher] ${reason} | Action Amount: ${amount}`);
  const txHash = executeOnChain(runtime2, action, price, amount);
  return { action, price, amount, reason, txHash, timestamp };
}
function calculateAmount(price) {
  const deviation = Math.abs(1 - price);
  const scale = BigInt(Math.floor(deviation / 1 * 1000000000000000000));
  const amount = DEFAULT_MAX_ACTION_AMOUNT * scale / BigInt(1000000000000000000);
  return amount > DEFAULT_MAX_ACTION_AMOUNT ? DEFAULT_MAX_ACTION_AMOUNT : amount;
}
function executeOnChain(runtime2, action, price, amount) {
  let contractAddr = "";
  try {
    const secretObj = runtime2.getSecret({ id: "PEG_DEFENDER_ADDRESS" }).result();
  ...
  try {
    const chainIdStr = runtime2.getSecret({ id: "CHAIN_ID" }).result().value;
    chainId = Number(chainIdStr || "11155111");
  } catch (e) {}
  const evmClient = new ClientCapability(BigInt(chainId));
  const price18 = BigInt(Math.floor(price * 1000000000000000000));
  const performData = encodeAbiParameters(parseAbiParameters("uint256 price, string actionType, uint256 amount"), [price18, action, amount]);
  const hexStringStr = performData.slice(2);
  const dataBytes = new Uint8Array(hexStringStr.length / 2);
  for (let i2 = 0;i2 < dataBytes.length; i2++) {
    dataBytes[i2] = parseInt(hexStringStr.slice(i2 * 2, i2 * 2 + 2), 16);
  }
  try {
    const reportResponse = runtime2.report(prepareReportRequest(bytesToHex(dataBytes))).result();
    ...
    if (writeResponse.txHash) {
      ...
      runtime2.log(`[action-dispatcher] EVM Write Successful! Tx: ${hash}`);
      return hash;
    }
    runtime2.log("[action-dispatcher] EVM Write prepared but not broadcast (Simulation mode).");
    return "0xSIMULATED_TRANSACTION";
  } catch (err) {
    runtime2.log(`[action-dispatcher] EVM Write failed: ${err.message}`);
    return;
  }
}
var onCronTrigger = (runtime2, _payload) => {
  runtime2.log("[stablearb-cre] Starting peg monitor run");
  const priceResult = fetchSusdPrice(runtime2);
  runtime2.log(`[stablearb-cre] Price: ${priceResult.price}`);
  const decision = dispatchAction(runtime2, priceResult);
  runtime2.log(`[stablearb-cre] Action: ${decision.action}`);
